...
def mk3im_bw(vfile,pcclip=5.0e6,pc=99.9):
    vhdu = fits.open(vfile)[0]
    vsum = vhdu.data.sum()
    thispc = pc
    if vsum >= pcclip:
        thispc = pcslope*(np.log10(vsum)-np.log10(pcclip))
    v0,vmed,vstd = getmedstd (vhdu.data)
    v = vhdu.data - vmed
    vpixi = pixi (v,vstd,pc=thispc)
    return vpixi
...
def list2npy_new(fl,nx,ny,prefix,cpix=50,cut=None,vsmooth=0.5,pcclip=5.0e6,pcslope=1.3,\
    ...
    while imn<len(fl):
        ...
        img_bw = np.zeros((ny*cpix,nx*cpix))
        for i in range(ny):
            for j in range(nx):
                ...
                img_bw[py:py+cpix,px:px+cpix] = mk3im_bw (fl[imn,0])
        ...
        np.save (prefix+'G%04d'%paneln,img_bw)
